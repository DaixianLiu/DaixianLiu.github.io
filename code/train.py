...
def maximum_square_loss(pred, target):
    diff = target - pred
...
def train(train_loader, val_loader, model, optimizer, epoch):
    ...
    maxSquareLoss_meter = AverageMeter()
    loss_meter = AverageMeter()
    intersection_meter = AverageMeter()
    union_meter = AverageMeter()
    target_meter = AverageMeter()
    
    diceLoss1 = AverageMeter()
    focalLoss1 = AverageMeter()
    ...
    sam_optimizer = SAM(model.parameters(), base_optimizer=torch.optim.SGD,lr=0.0001,rho=0.05, adaptive=True)
    ...
    for i, (input, target, target_b, s_input, s_mask, subcls) in enumerate(train_loader):
        ...
        output, main_loss, aux_loss1, aux_loss2 = model(s_x=s_input, s_y=s_mask, x=input, y_m=target, y_b=target_b,
                                                        cat_idx=subcls)
        maxSquareLoss = maximum_square_loss(output, target)
        diceLoss = dice_loss(output, target)
#         focalLoss = focal_loss(output, target)
        focalLoss=0
        loss = main_loss + args.aux_weight1 * aux_loss1 + args.aux_weight2 * aux_loss2+ 0.1*diceLoss
        ...
        maxSquareLoss_meter.update(maxSquareLoss.item(), n)
        loss_meter.update(loss.item(), n)
        diceLoss1.update(diceLoss.item(), n)
#         focalLoss1.update(focalLoss.item(), n)
        ...
        if (i + 1) % args.print_freq == 0 and main_process():
            logger.info('Epoch: [{}/{}][{}/{}] '
                        ...
                        'maxSquareLoss {maxSquareLoss_meter.val:.4f} ' 
#                         'focalLoss1{focalLoss1.val:.4f} ' 
                        'diceLoss1 {diceLoss1.val:.4f} ' 
...
#                                                         focalLoss1=focalLoss1,
                                                        diceLoss1=diceLoss1,
                                                        maxSquareLoss_meter=maxSquareLoss_meter,
